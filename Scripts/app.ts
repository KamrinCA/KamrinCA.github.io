 ...
 * Date Completed: March 22th, 2022
 * Description: app.js file. Contains multiple functions that help support
 *              the website being a SPA (Single-Page Application) with the help
 *              AJAX.
 *
 */
...
{
    /**
     * Handles authentication for certain pages/views only being accessible if a
     * user is signed in
     * 
     * @returns {void}
     */
    function AuthGuard(): void
    {
        let protected_routes: string[] = [
            "contact-list",
            "task-list" // Added the task-list to the protect routes
        ];
    ...
    /**
     * Loads the content of each view when passed a link (The link is a string to each page). 
     * Data is optional
     * 
     * @param {string} link
     * @param {string} [data=""]
    ...
    /**
     * Adds navigation events for all navigation links in the navbar
     *
     * @returns {void}
     */
    ...
    /**
     * Handles the links for buttons and other links placed on the main content sections
     * of the SPA
     *
     * @param {string} link
     * @returns {void}
    ...
    /**
     * Loads the content of each page/view
     * 
     * @returns {void}
     */
    ...
    /**
     * Loads the footer on each page/view
     * 
     * @returns {void}
     */
    ...
    /**
     * Function that manipulates the SPA when a user is logged in
     *
     * @returns {void}
     */
    function CheckLogin(): void
    {
        ...
        if(sessionStorage.getItem("user"))
        {
            ...
            // Creates the new nav link for task list
            let taskList = document.createElement("li") as HTMLElement;
            taskList.setAttribute("id", "task-list");
            taskList.setAttribute("class", "nav-item");
            taskList.innerHTML = `<a class="nav-link" data="task-list" style="cursor: pointer;"><i class="fas fa-tasks"></i> Task-list</a>`;

            // Once the document is loaded/ready
            $.when( $.ready ).then(function() 
            {
                // Removes the task list to prevent duplicates from appearing in navbar
                $("#task-list").remove();
                // Places the tasklist in the navbar after "contact"
                $("ul").find("li")[4].after(taskList);
                // Turns off the click event handler on the link
                $("#task-list").off("click");
                // Lastly, adds the click event handler for the nav link
            ...
            $("#logout").on("click", function()
            {
                ...
                $("#login").html(
                    `<a class="nav-link" data="login" style="cursor: pointer;"><i class="fas fa-sign-in-alt"></i> Login</a>`
                );
    ...
    {   
        switch(router.ActiveLink)
        {
            ...
            case "task-list": return DisplayTaskList;   // Added task-list to switch-case, passing a reference to the DisplayTaskList function
            default:
